...
class CrudManager extends BddConnect
{
  ...
  {
    // SQL $req = $this->_dbConnect->prepare("SELECT * FROM " . $this->_tableName . " WHERE {$columnLike} LIKE :search AND JSON_CONTAINS(`jsonFood`, :json) LIMIT 10");
    $req = $this->_dbConnect->prepare("SELECT * FROM {$this->_tableName}  WHERE {$columnLike} LIKE :search AND JSON_SEARCH(JSON_EXTRACT(`jsonFood`, :json)) LIMIT 10");
